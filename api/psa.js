...
function jinaFetch(cert) {
  const headers = {
    ...
    // บังคับ Jina เรนเดอร์สด — บางที Jina แคชหน้าเวอร์ชันที่รูป cert ยังไม่มา
    // ไว้ (ตอบว่า "ไม่มีรูป" ทั้งที่ PSA มีรูป) การ render สดจะได้รูปครบ
    // ไม่เพิ่มโหลด: cert ที่สำเร็จแล้วแคชใน Blob ของเราเอง → no-cache จ่ายแค่ครั้งแรกต่อ cert
    'X-No-Cache': 'true',
    'X-Timeout': '12',
    Accept: 'text/html,*/*',
  };
