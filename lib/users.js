...
export async function verify(id, pass, opts) {
  ...
  if (id === ADMIN_ID && pass === process.env.APP_PASSCODE) {
    return { ok: true, id, role: 'admin', tabs: ALL_TABS, seeProfit: true, gachaReadonly: false, buyEdit: true };
  }
  const users = await loadUsers(opts);
  const u = users.find(x => x.id === id && x.password === pass);
  if (u) return { ok: true, id: u.id, role: 'assistant', tabs: Array.isArray(u.tabs) ? u.tabs : [], seeProfit: u.seeProfit !== false, gachaReadonly: u.gachaReadonly === true, buyEdit: u.buyEdit === true };
  return { ok: false };
}
