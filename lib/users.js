...
export async function verify(id, pass, opts) {
  ...
  if (id === ADMIN_ID && pass === process.env.APP_PASSCODE) {
    return { ok: true, id, role: 'admin', tabs: ALL_TABS, seeProfit: true, gachaEdit: true, buyEdit: true, shops: [], shopFeats: {} };
  }
  const users = await loadUsers(opts);
  const u = users.find(x => x.id === id && x.password === pass);
  if (u) return { ok: true, id: u.id, role: 'assistant', tabs: Array.isArray(u.tabs) ? u.tabs : [], seeProfit: u.seeProfit !== false, gachaEdit: u.gachaEdit === true, buyEdit: u.buyEdit === true, shops: Array.isArray(u.shops) ? u.shops : [], shopFeats: (u.shopFeats && typeof u.shopFeats === 'object') ? u.shopFeats : {} };
  return { ok: false };
}
